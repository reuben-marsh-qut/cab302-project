...
### Overview & Goals
When running the application via `Launcher.java`, `HelloApplication` loads `home-view.fxml`. However, the label defined in `home-view.fxml` (`"Select a contact to view or edit."`) is not visible on the screen. The goal is to resolve this issue so that the label is positioned correctly within the layout and renders as expected when the application is launched.
...
- When `Launcher.main()` runs, `HelloApplication` loads `home-view.fxml`.
...
- `HelloApplication.java` creates a `Scene` by loading `home-view.fxml`:
  ```java
...
- In `src/main/resources/com/example/cab302project/home-view.fxml`:
  ...
  <BorderPane xmlns="http://javafx.com/javafx"
              ...
              fx:controller="com.example.cab302project.controller.HomeController"
...
### Validation Approach
- Verify FXML parsing of `home-view.fxml` to confirm the root `BorderPane` contains the `Label` in its `center` property.
...
The label in `home-view.fxml` is properly assigned to the center slot of `BorderPane`.
...
- Clean up any unused imports or redundant whitespace in `home-view.fxml` to ensure valid FXML structure.
...
- Verify that `FXMLLoader` loads `home-view.fxml` correctly in `HelloApplication.java`.
